
<html>
<head>
<title>Sunil Patil Portfolio</title>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="https://www.w3schools.com/w3css/5/w3.css">
</head>
<body class="w3-content" style="max-width:1300px">

<!-- First Grid: Logo & About -->
<div class="w3-row">
  <div class="w3-half w3-black w3-container w3-center" style="height:700px">
    <div class="w3-padding-64">
      <h1>Sunil Patil</h1>
    </div>
    <div class="w3-padding-64">
      <a href="#" class="w3-button w3-black w3-block w3-hover-blue-grey w3-padding-16">Home</a>
      <a href="#work" class="w3-button w3-black w3-block w3-hover-teal w3-padding-16">My Work</a>
      <a href="#resume" class="w3-button w3-black w3-block w3-hover-dark-grey w3-padding-16">Resume</a>
      <a href="#contact" class="w3-button w3-black w3-block w3-hover-brown w3-padding-16">Contact</a>
    </div>
  </div>

  <div class="w3-half w3-blue-grey w3-container" style="height:700px">
    <div class="w3-padding-64 w3-center">
      <h1>About Me</h1>
      <img src="mypic.jpg" class="w3-margin w3-circle" alt="Person" style="width:50%">
      <div class="w3-left-align w3-padding-large">
        <p>Highly organized Accounts Payable and Finance Professional with 3+ years of experience in invoice processing, vendor management, financial reconciliation, and migration support across global operations. Proficient in SAP (EUC1, HANA, VIM), JDE, Power BI, Ariba Portal, IBM Datacap, and ServiceNow, with a strong track record of accuracy, compliance, and stakeholder collaboration.</p>
        
      </div>
    </div>
  </div>
</div>

<!-- Second Grid: Work & Resume -->
<div class="w3-row">
  <div class="w3-half w3-light-grey w3-center" style="min-height:800px" id="work">
    <div class="w3-padding-64">
      <h2>My Work</h2>
      <p>Some of my latest projects.</p>
    </div>
    <div class="w3-row">
      <div class="w3-half"><img src="image 1.webp" style="width:100%"></div>
      <div class="w3-half"><img src="image 2.webp" style="width:100%"></div>
      <div class="w3-half"><img src="image 3.webp" style="width:100%"></div>
      <div class="w3-half"><img src="image 4.webp" style="width:100%"></div>
      <div class="w3-half"><img src="image 5.webp" style="width:100%"></div>
      <div class="w3-half"><img src="image 6.webp" style="width:100%"></div>
    </div>
  </div>

  <div class="w3-half w3-indigo w3-container" style="min-height:800px" id="resume">
    <div class="w3-padding-64 w3-center">
      <h2>Resume</h2>
      <p>Professional Experience & Education</p>
      <div class="w3-container w3-responsive">
        <table class="w3-table w3-bordered w3-white">
          <tr>
            <th>Year</th>
            <th>Role / Education</th>
            <th>Organization</th>
          </tr>
          <tr>
            <td>Jan 2025 – Present</td>
            <td>Invoice to Pay Process Associate</td>
            <td>Cargill Business Services</td>
          </tr>
          <tr>
            <td>Apr 2023 – Dec 2024</td>
            <td>Invoice to Pay Process Assistant</td>
            <td>Cargill Business Services</td>
          </tr>
          <tr>
            <td>May 2022 – Nov 2022</td>
            <td>Assistant</td>
            <td>Chartered Accountant Firm</td>
          </tr>
          <tr>
            <td>2021 – 2022</td>
            <td>MBA (Finance & Marketing)</td>
            <td>Reva Institute of Science and Management</td>
          </tr>
          <tr>
            <td>2017 – 2020</td>
            <td>BBM (Finance & Marketing)</td>
            <td>SB College of Commerce</td>
          </tr>
        </table>
      </div>
    </div>
  </div>
</div>

<!-- Third Grid: Contact -->
<div class="w3-row" id="contact">
  <div class="w3-half w3-dark-grey w3-container w3-center" style="height:700px">
    <div class="w3-padding-64">
      <h1>Reach Me</h1>
    </div>
    <div class="w3-padding-64">
      <p>Bengaluru, Karnataka</p>
      <p>+91 9590693138</p>
      <p>sunilptl173@gmail.com</p>
      <p>LinkedIn | Portfolio</p>
    </div>
  </div>

  <div class="w3-half w3-teal w3-container" style="height:700px">
    <div class="w3-padding-64 w3-padding-large">
      <h1>Contact</h1>
      <p class="w3-opacity">Get in touch</p>
      <form class="w3-container w3-card w3-padding-32 w3-white" action="#" target="_blank">
        <div class="w3-section">
          <label>Name</label>
          <input class="w3-input" style="width:100%;" type="text" required>
        </div>
        <div class="w3-section">
          <label>Email</label>
          <input class="w3-input" style="width:100%;" type="email" required>
        </div>
        <div class="w3-section">
          <label>Message</label>
          <textarea class="w3-input" style="width:100%;" required></textarea>
        </div>
        <button type="submit" class="w3-button w3-teal w3-right">Send</button>
      </form>
    </div>
  </div>
</div>

<!-- Footer -->
<footer class="w3-container w3-black w3-padding-16">
  <p>Powered by w3.css</p>
</footer>

</body>
</html>
